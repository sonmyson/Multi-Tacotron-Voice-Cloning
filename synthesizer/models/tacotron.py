...
class Tacotron():
    ...
    def initialize(self, inputs, input_lengths, embed_targets, mel_targets=None, 
        ...
        with tf.compat.v1.device(split_device):
            hp = self._hparams
            ...
            p_inputs = tf.py_function(split_func, [inputs, split_infos[:, 0]], lout_int)
    ...
    def add_loss(self):
        ...
        for i in range(hp.tacotron_num_gpus):
            with tf.compat.v1.device(tf.train.replica_device_setter(ps_tasks=1, ps_device="/cpu:0",
                                                          worker_device=gpus[i])):
    ...
    def add_optimizer(self, global_step):
        ...
        with tf.compat.v1.device(grad_device):
